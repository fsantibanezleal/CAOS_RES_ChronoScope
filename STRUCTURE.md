...
> generic names. In the tree actually shipped: **`productlab/` = `data-pipeline/chronoscopelab/`** (the Python
> package lives INSIDE `data-pipeline/`; on instantiation rename `chronoscopelab` → `<slug>lab`), **`web/` =
