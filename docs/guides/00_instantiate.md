...
2. **Rename** the package `chronoscopelab` → `<slug>lab` (the folder + all imports + `pyproject.toml`
   `[tool.setuptools.packages.find].where`/name + the scripts' `-m chronoscopelab.pipeline` + docs).
