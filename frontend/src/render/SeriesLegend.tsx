    ...
    <div className="cs-legend-panel">
      ...
      <div className="cs-legend-metric" role="tablist" aria-label={es ? 'métrica mostrada' : 'displayed metric'}>
        {METRICS.map((m) => (
          <button
            ...
            title={!has(m.id)
              ? (es ? 'métrica horneada: elige un caso horneado' : 'baked metric: pick a baked case')
              : m.id === 'mase'
      ...
      <div className="cs-legend-foot">
        {metric === 'mase'
          ...
          : metric === 'wql'
            ? (es ? 'WQL: pérdida pinball sobre los cuantiles horneados, normalizada; puntúa la DISTRIBUCIÓN.' : 'WQL: pinball loss over the baked quantiles, normalized; scores the DISTRIBUTION.')
