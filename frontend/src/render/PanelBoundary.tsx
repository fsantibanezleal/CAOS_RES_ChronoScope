// Per-panel error boundary: one broken panel must never blank the whole app (an edge-case source
// producing NaN geometry once crashed a sibling product page-wide; this is the standing guard).
// Class component because React error boundaries have no hook equivalent.
import { Component, type ReactNode } from 'react';

interface Props {
  /** Panel name shown in the fallback so the failure is identifiable in a screenshot. */
  label: string;
  es?: boolean;
  children: ReactNode;
}
...
export class PanelBoundary extends Component<Props, State> {
  ...
  componentDidUpdate(prev: Props) {
    // A new child tree (tab/case switch) gets a fresh chance instead of a sticky fallback.
  ...
  render() {
    ...
    if (error) {
      return (
        <div className="cs-panel" role="alert">
