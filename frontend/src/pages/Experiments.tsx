...
export default function Experiments() {
  ...
  return (
    <section className="page-body prose">
      ...
      <Equation tex={String.raw`\text{MSIS} = \frac{\frac{1}{h}\sum_{i=1}^{h}\Big[(u_i - l_i) + \tfrac{2}{\alpha}(l_i - y_i)\,\mathbf{1}\{y_i < l_i\} + \tfrac{2}{\alpha}(y_i - u_i)\,\mathbf{1}\{y_i > u_i\}\Big]}{\frac{1}{T-m}\sum_{t=m+1}^{T} |y_t - y_{t-m}|}`} caption={es ? 'MSIS (Gneiting y Raftery 2007; la métrica de intervalo de M4, con α = 0.2 para la banda del 80%): ancho del intervalo más una multa de 2/α por unidad de falla, escalado como MASE. Pone el trade-off ancho-vs-cobertura en UN número: un intervalo angosto que falla no puede verse bien.' : 'MSIS (Gneiting & Raftery 2007; the M4 interval metric, with α = 0.2 for the 80% band): interval width plus a 2/α penalty per unit of miss, scaled like MASE. It prices the width-vs-coverage trade-off in ONE number: a narrow interval that misses cannot look good.'} />
      <p>{es
