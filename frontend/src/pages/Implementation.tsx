...
export default function Implementation() {
  ...
  return (
    <section className="page-body prose">
      ...
      <p>{es
        ? 'Offline: el pipeline por etapas (preprocess, analyze, feature_extraction, train, infer, evaluate + streaming, export) se ejecuta en Python 3.12 con la GPU (torch cu126) y precalcula, por caso: la traza de pronósticos (19 métodos con backtest), el panel de análisis (10 familias) y el banco de streaming (trayectorias prequential). Replay: la web lee esos artefactos; nada pesado se ejecuta aquí. En vivo: la escalera clásica está portada a TypeScript y verificada por paridad contra el núcleo Python sobre un fixture comprometido (el navegador computa lo mismo que el pipeline, no una aproximación decorativa), y un NLinear exportado a ONNX se ejecuta con onnxruntime-web.'
      ...
      <svg viewBox="0 0 680 130" width="100%" style={{ maxWidth: 680, display: 'block', margin: '0.8rem auto', font: '10px var(--font-sans, sans-serif)' }} role="img" aria-label={es ? 'las etapas del pipeline y los artefactos que emite' : 'the pipeline stages and the artifacts they emit'}>
        ...
        <text x="6" y="14" fill="var(--color-fg)" fontSize="10">{es ? 'una ejecución = una función pura de (caso, semilla); cada etapa es tipada, sembrada y testeada' : 'one run = a pure function of (case, seed); every stage is typed, seeded, and tested'}</text>
      ...
      <Callout variant="note" title={es ? 'Reproducibilidad' : 'Reproducibility'}>
        {es
          ? 'Todo es reproducible desde el repo: requirements fijados por carril (vivo = solo numpy; precompute = el stack completo), el pipeline se invoca con una semilla, y los artefactos comprometidos coinciden byte a byte al re-precalcular. preqts, el paquete de evaluación streaming extraído de este trabajo, está publicado en PyPI (pip install preqts).'
