...
const CONTRACTS = svg('cs-a5', 430, `
  ...
  <text class="mu" x="32" y="162">accepts YOUR data, not just cases</text>
...
export const architecture: ArchitectureConfig = {
  ...
  tabs: [
    ...
    },
    {
      ...
      body_es: 'La página App es un banco de trabajo por caso. Un selector de fuente elige un caso horneado (replay) o un patrón sintético con perillas en vivo; los motores producen pronósticos (en vivo para clásicos + ONNX, replay para los niveles pesados); y el caso se presenta en 10 pestañas interactivas, divididas ENTENDER (Serie, Estructura, Descomponer, Veredictos) y PRONOSTICAR (Pronóstico con un toggle Pronóstico/Errores, Zoom, Horizonte, Residuos, Tabla, Streaming). Dos mecanismos lo mantienen honesto: copy-data.mjs superpone los artefactos versionados (nunca tipeados a mano) y el espejo de contrato en TypeScript rompe el build ante cualquier divergencia de esquema; un drift-gate de CI corre antes de desplegar.',
    },
    {
